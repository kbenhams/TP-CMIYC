...
elementsDOM.startDOM.addEventListener("click", startGame, {
  once: true
});
...
function startGame() {
  ...
  elementsDOM.rotatingDOM.style.animationDuration = elementsGame.rotateDur + "s";
...
function createHightScore() {
  ...
  names.forEach(name => {
    toAppend += 
    `<div class="users" class="tooltip">
    <div class="date" class="tooltiptext">${name.date}</div>
